...
import { DeparturesResponse, StationDeparturesResponse } from '../types/departures';
...
export const useApiRequest = () => {
    ...
    const makeRequest = useCallback(async <T>(
        ...
        const requestWithRetry = async (attemptsLeft: number): Promise<T> => {
            try {
    ...
    const fetchDepartures = useCallback(async (lat: number, lon: number): Promise<DeparturesResponse> => {
    ...
    const fetchStationDepartures = useCallback(async (stationId: string): Promise<StationDeparturesResponse> => {
