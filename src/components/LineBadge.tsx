...
export function LineBadge({
    ...
    return (
        <Badge
            variant="outline"
            className={cn(
                'flex shrink-0 items-center justify-center rounded-full border-0 bg-muted p-0 leading-none font-semibold tracking-normal text-foreground data-[line=capital]:bg-line-capital data-[line=capital]:text-line-on-colour data-[line=metro]:bg-line-metro data-[line=metro]:text-line-on-amber data-[line=valley]:bg-line-valley data-[line=valley]:text-line-on-colour',
                proportional
                    ? 'line-badge-proportional'
                    : 'size-4.5 text-[10px] data-hero:size-5.5 data-hero:text-xs [&_svg]:size-3.5',
                className
            )}
