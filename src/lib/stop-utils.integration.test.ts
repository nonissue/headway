...
describe('GTFS departure windows', () => {
    ...
    it('excludes passed departures at the exact boundary across service dates', async () => {
        trip('due', 20260919, '29:00:00');
        ...
        vi.setSystemTime(new Date('2026-09-20T11:00:00.001Z'));
...
describe('next-service fallback', () => {
    ...
    it('excludes past departures when the board falls back to morning service', async () => {
        vi.setSystemTime(new Date('2026-09-20T05:30:00Z'));
        trip('recent', 20260919, '23:28:00');
        trip('expired', 20260919, '23:26:00');
        trip('morning', 20260920, '05:55:00');
        const result = await getDeparturesForStation('s');
        expect(result.nextServiceAt).toBe('2026-09-20T11:55:00.000Z');
        expect(ids(result)).toEqual(['morning']);
    });
